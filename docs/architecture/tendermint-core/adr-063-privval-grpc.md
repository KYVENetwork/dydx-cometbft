...
```proto
  service PrivValidatorAPI {
    rpc GetPubKey(dydxcometbft.proto.privval.PubKeyRequest) returns (dydxcometbft.proto.privval.PubKeyResponse);
    rpc SignVote(dydxcometbft.proto.privval.SignVoteRequest) returns (dydxcometbft.proto.privval.SignedVoteResponse);
    rpc SignProposal(dydxcometbft.proto.privval.SignProposalRequest) returns (dydxcometbft.proto.privval.SignedProposalResponse);
  ...
  message PubKeyResponse {
    dydxcometbft.crypto.PublicKey pub_key = 1 [(gogoproto.nullable) = false];
  }

  // SignVoteRequest is a request to sign a vote
  message SignVoteRequest {
    dydxcometbft.types.Vote vote     = 1;
    string                chain_id = 2;
  }

  // SignedVoteResponse is a response containing a signed vote or an error
  message SignedVoteResponse {
    dydxcometbft.types.Vote vote  = 1 [(gogoproto.nullable) = false];
  }

  // SignProposalRequest is a request to sign a proposal
  message SignProposalRequest {
    dydxcometbft.types.Proposal proposal = 1;
    string                    chain_id = 2;
  }

  // SignedProposalResponse is response containing a signed proposal or an error
  message SignedProposalResponse {
    dydxcometbft.types.Proposal proposal = 1 [(gogoproto.nullable) = false];
  }
}
```
